...
def getOpenConnectionAWS(host, database="postgres"):
    return psycopg2.connect(host=host, port=5432, database=database, user="postgres", password="12345678")

def createDBAWS():
    #Connect to default database
    for i in range(5):
        con = getOpenConnectionAWS(host[i])
        con.set_isolation_level(psycopg2.extensions.ISOLATION_LEVEL_AUTOCOMMIT)
        cur = con.cursor()
